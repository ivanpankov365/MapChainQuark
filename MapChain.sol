...
contract MapChain {
    ...
    mapping(uint256 => string) task; //number_task => task
    mapping(address => Leaser) leasers;
    
    function MapChain (uint256 _price, string _task1, string _task2, string _task3) public {
        balance=0;
        amountForOwner=0;
        game_status=false;
        owner = msg.sender;
    ...
    function Task() public view returns (string) {
        require (game_status==true);
        return task[current_steps];
    }
